...
######################################################
# Build up the arguments for the oc command
######################################################
...
if [ -s ./ca.crt ]; then
  SECRET_ARGS="${SECRET_ARGS} --from-file=ca.crt=./ca.crt "
fi

if [ -s ./tls.cert ]; then
  SECRET_ARGS="${SECRET_ARGS} --from-file=tls.cert=./tls.cert "
fi

if [ -s ./tls.key ]; then
  SECRET_ARGS="${SECRET_ARGS} --from-file=tls.key=./tls.key "
fi
...
echo "oc create secret generic ${TARGET_SECRET_NAME}${SECRET_ARGS}"
oc create secret generic ${TARGET_SECRET_NAME}${SECRET_ARGS} --dry-run -o yaml | oc apply -f -
